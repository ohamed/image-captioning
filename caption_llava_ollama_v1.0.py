...
def process_excel(
    ...
    # Create caption_ai column if it doesn't exist
    ...
    for idx, row in df.iterrows():
        ...
        if pd.notna(existing) and str(existing).strip() and not overwrite:
            print(f"[{idx + 1:3d}] SKIP (exists): {image_id}")
            skipped += 1
            continue

        # Build image path using pathlib (Windows-safe)
        image_path = img_root / category / str(image_id)

        # Check if image exists
        if not image_path.exists():
            print(f"[{idx + 1:3d}] MISSING: {image_path}")
